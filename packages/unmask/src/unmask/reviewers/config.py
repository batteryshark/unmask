...
# base_url + which env var holds the api key, and the wire protocol `kind`:
# "openai" (chat-completions) or "anthropic" (messages). Most endpoints speak OpenAI;
# MiniMax's coding-plan works best on its Anthropic-compatible endpoint (cleaner reasoning
# + tool use than its /v1 chat-completions). Override any field via env.
_PRESETS = {
    "lmstudio": {"base_url": "http://localhost:1234/v1", "api_key_env": "UNMASK_REVIEW_API_KEY", "local": True, "kind": "openai"},
    "minimax": {"base_url": "https://api.minimax.io/anthropic", "api_key_env": "MINIMAX_API_KEY", "local": False, "kind": "anthropic"},
    "zai": {"base_url": "https://api.z.ai/api/paas/v4", "api_key_env": "ZAI_API_KEY", "local": False, "kind": "openai"},
    "openai": {"base_url": "https://api.openai.com/v1", "api_key_env": "OPENAI_API_KEY", "local": False, "kind": "openai"},
    "anthropic": {"base_url": "https://api.anthropic.com", "api_key_env": "ANTHROPIC_API_KEY", "local": False, "kind": "anthropic"},
}
...
class ReviewModelConfig:
    ...
    api_key: str | None
    provider: str
    kind: str = "openai"        # wire protocol: "openai" (chat-completions) or "anthropic" (messages)
    ...
    def from_env(cls, *, model=None, base_url=None, api_key=None, provider=None) -> "ReviewModelConfig":
        ...
        # wire protocol: explicit env wins, else the preset's, else default openai. Lets a
        # `custom` provider hit an Anthropic-compatible endpoint via UNMASK_REVIEW_KIND=anthropic.
        kind = os.environ.get("UNMASK_REVIEW_KIND") or (preset.get("kind") if preset else None) or "openai"

        if not model:
            raise ReviewConfigError(
                "no review model configured — set UNMASK_REVIEW_MODEL (and a base_url via "
                "UNMASK_REVIEW_BASE_URL or UNMASK_REVIEW_PROVIDER=lmstudio|minimax|zai|openai|anthropic)."
            )
        if not base_url:
            raise ReviewConfigError(
    ...
    def build_model(self):
        """Construct the pydantic-ai model — an Anthropic `messages` model or an OpenAI
        chat-completions model, per `kind`. Both take a base_url + api_key, so any
        OpenAI- or Anthropic-compatible endpoint works."""
        try:
