...
from .rules import (_ts_available, _parser, _GRAMMAR, _v, _node_kind,
                    _node_children, _node_text, _field, _callee_text)
from .dataflow import _sink_kinds, DATAFLOW_LANGS
...
def _resolve_spec(spec: str, importer_rel: str, file_set):
    ...
    if spec.startswith("."):
        root = os.path.normpath(os.path.join(base, spec.lstrip(".") and spec or "."))
...
def _collect_functions(root, data):
    ...
    # named defs anywhere in the file
    stack = [(root, True)]
    while stack:
        n, top = stack.pop()
        k = _node_kind(n)
...
def analyze(inv) -> dict:
    ...
    files = [f for f in getattr(inv, "files", []) if getattr(f, "lang", None) in DATAFLOW_LANGS]
    if not _ts_available() or not files:
        return {}

    file_set = {f.relpath for f in files}
    ...
    for f in files:
        try:
            with open(f.abspath, encoding="utf-8", errors="replace") as fh:
                src = fh.read()
            parser = _parser(_GRAMMAR.get(f.lang, f.lang))
            try:
        ...
        parsed[f.relpath] = (functions, module_calls, _import_map(src, f.lang), data)
        funcs_by_file[f.relpath] = {name for name, _ in functions}
