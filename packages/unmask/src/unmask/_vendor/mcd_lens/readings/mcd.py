"""The MCD reading: is this malicious? Composes atoms into BP-* bad-pattern compositions."""

from __future__ import annotations

from engine.interpret.common import *  # noqa: F401,F403 (shared finding/severity/proof helpers)


def _mcd_strong_agent_steering(o) -> bool:
    # A bare tool/MCP declaration is an agent surface, not manipulation by itself.
    # MCD reserves BP-AGENTMANIP for hidden/injection/deception content next to
    # capability, while agent-risk owns the broader "this is a tool surface" view.
...
def mcd(obs, inv=None) -> list:
    ...
    for inst in installs:
        ...
        for t in files:
            grp = groups.get(t, [])
            # MCD reserves its malicious verdict for unambiguous install-time payloads:
            # network egress, or decode/decrypt-and-execute. Bare install-time exec
            # (e.g. a C-extension compiler step) is genuinely ambiguous from static
            # source, so the capability lens flags it (CAP-INSTALL) without claiming malice.
            payload += _has(grp, "NETW")
            if _has(grp, "LOAD.EVAL") and _has(grp, "XFRM"):  # install-time decode/decrypt-and-execute
    ...
    # Name similarity is a real but local-only signal, so it is medium on its own
    # and high when it ships a payload. Confirmation needs the registry (osint).
    ...
    # Kept below the proven-path confidence bar unless the surrounding context
    # corroborates persistence/evasion/concealment; static source sees shape, not
    # authorization.
    for path, group in groups.items():
    ...
    # BP-EXFIL: sensitive data collection + outbound channel, broader than pure
    # credential theft. Avoid duplicating CRED+NETW here; BP-CREDTHEFT owns that
    # sharper chain. Plain host/user/process telemetry is intentionally left to
    # capability/curiosity unless paired with more sensitive collection.
    for path, group in groups.items():
    ...
    # BP-RANSOM: enumerate files, encrypt/write/delete them, optionally leave a
    # command or note artifact. This is intentionally specific to avoid flagging
    # ordinary crypto libraries.
    for path, group in groups.items():
    ...
    # BP-TIMEBOMB: time or environment trigger gates a payload.
    for path, group in groups.items():
    ...
    # BP-ROOTKIT: system-level hooks, injection, kernel modules, boot persistence,
    # or security disablement with concealment.
    for path, group in groups.items():
    ...
    # BP-WORM: discover targets, then deliver or execute across a network.
    # Require network/host discovery, not just ordinary local user/process facts.
    for path, group in groups.items():
    ...
    # BP-TROJAN: a concealed payload in a component whose stated purpose does not
    # suggest that behavior. The purpose heuristic is intentionally weak, so keep
    # confidence below quarantine on its own.
    ...
    # BP-AGENTMANIP: MCD interpretation of agent-directed manipulation when it
    # sits next to a damaging payload surface. The agentic lens owns broader
    # steerability; MCD fires only when payload atoms are present too.
