"""The deterministic quarantine / review / clear rule + executive summary.

Ported from `mcd_lens.assess.disposition`. Severity and confidence are weighed
by an explicit rule, never collapsed into one score and never set by a model.
"""
...
def _review_leads(reachability=None):
    ...
    return [{
        ...
        "claim": ("A package entry reaches network fetch or egress and code execution across files. "
        ...
        "evidence": [
            {"file": s.get("file"), "function": s.get("function"),
             "sinkKinds": list(s.get("sinkKinds") or []), "entryFile": s.get("entryFile"),
